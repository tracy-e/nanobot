...
class ClaudeCodeProvider(LLMProvider):
    ...
    async def _call_claude(self, message: str, session_id: str | None) -> tuple[str, str | None]:
        ...
        try:
            # Build a clean env: remove ANTHROPIC_API_KEY so claude CLI
            # uses its own auth (e.g. Max subscription) instead of a key
            # that other providers may have injected.
            ...
            if proc.returncode != 0:
                ...
                logger.error(f"claude CLI failed (rc={proc.returncode}): stderr={err}, stdout={out[:500]}")
                raise ClaudeCodeError(f"claude CLI exited with code {proc.returncode}: {err or out[:200]}")
